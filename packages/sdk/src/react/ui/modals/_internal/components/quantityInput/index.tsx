...
const _default_1: ({ $quantity, $invalidQuantity, decimals, maxQuantity, className, }: QuantityInputProps) => JSX.Element = observer(function QuantityInput({
    $quantity, $invalidQuantity, decimals, maxQuantity, className,
}: QuantityInputProps) {
    ...
    return (
        ...
        >
            <NumericInput
                ...
                labelLocation="top"
                controls={<div className="mr-2 flex items-center gap-1">
                    <IconButton
                ...
                numeric={true}
                value={localQuantity}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChangeQuantity(e.target.value)}
                width={'full'} />
            {$invalidQuantity.get() && (
