...
import { getMarketplaceClient, MarketplaceKind } from "../../../../_internal";
...
export const useCheckoutOptions = (input: {
    ...
    const fees = useFees({ chainId: input.chainId, collectionAddress: input.collectionAddress });

    return useQuery({
        queryKey: ['checkoutOptions', input.chainId, input.collectionAddress, input.orderId, input.marketplace],
        queryFn: async () => {
