...
describe('getPublicRpcClient', () => {
  describe('successful client creation', () => {
    test('should create client for mainnet', () => {
      ...
      expect((client as PublicClient & { chain: { id: number } }).chain.id).toBe(chainId);
      expect((client as PublicClient & { chain: { name: string } }).chain.name).toBe('mainnet');
    });
    ...
    test('should create client for polygon', () => {
      ...
      expect(client).toBeDefined();
      expect((client as PublicClient & { chain: { id: number } }).chain.id).toBe(chainId);
      expect((client as PublicClient & { chain: { name: string } }).chain.name).toBe('polygon');
    });
    ...
    test('should set up multicall batching', () => {
      ...
      expect((client as PublicClient & { batch: { multicall: boolean } }).batch.multicall).toBe(true);
    });
  ...
  describe('error handling', () => {
    test('should throw MissingConfigError for invalid chainId', () => {
      ...
      expect(() => getPublicRpcClient(invalidChainId)).toThrow(MissingConfigError);
