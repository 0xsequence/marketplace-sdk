...
const marketplaceGenPath = join(
	__dirname,
	'../sdk/src/react/_internal/api/marketplace.gen.ts',
);
...
try {
	...
	const versionLineRegex =
		/^(export const WebrpcHeaderValue =\s*")([^"]+)(";?)$/m;

	const match = marketplaceContent.match(versionLineRegex);
	if (!match) {
		console.error(
			'Could not find WebrpcHeaderValue line in marketplace.gen.ts',
		);
		process.exit(1);
	}
	...
	console.log(
		`✅ Updated WebrpcHeaderValue with marketplace-sdk@v${sdkVersion}`,
	);
} catch (error) {
