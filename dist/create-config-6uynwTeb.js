...
import { MissingConfigError } from "./transaction-CC2KxNF6.js";
import { apple, appleWaas, coinbaseWallet, ecosystemWallet, email, emailWaas, facebook, getConnectWallets, getDefaultChains, google, googleWaas, sequence, twitch, walletConnect } from "@0xsequence/connect";
...
function getAllCollections(marketConfig) {
	return [...marketConfig.market.collections, ...marketConfig.shop.collections];
}
function getChainConfigs(marketConfig) {
	const supportedChainIds = new Set(getAllCollections(marketConfig).map((c) => c.chainId));
	if (supportedChainIds.size === 0) supportedChainIds.add(DEFAULT_NETWORK);
...
//# sourceMappingURL=create-config-6uynwTeb.js.map