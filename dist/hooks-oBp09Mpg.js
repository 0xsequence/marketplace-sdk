...
import { collectableKeys, getIndexerClient, getMarketplaceClient, getQueryClient } from "./api-CBE1qhk6.js";
import { CollectionStatus, ContractType, ExecuteType, OrderSide, PropertyType, StepType, WalletKind } from "./marketplace.gen-JzNYpM0U.js";
...
import { Button, Modal, Spinner, Text, ThemeProvider, WarningIcon } from "@0xsequence/design-system";
import { jsx, jsxs } from "react/jsx-runtime";
import { BaseError as BaseError$1, TransactionReceiptNotFoundError, UserRejectedRequestError as UserRejectedRequestError$1, WaitForTransactionReceiptTimeoutError, custom, erc20Abi, erc721Abi, hexToBigInt, isHex, zeroAddress } from "viem";
...
//#endregion
//#region src/react/hooks/data/tokens/useSearchMintedTokenMetadata.tsx
/**
...
* ```
*/
function useSearchMintedTokenMetadata(params) {
	const defaultConfig = useConfig();
	const { config = defaultConfig, chainId, collectionAddress,...rest } = params;
	const { data: tokenSupplies } = useTokenSupplies({
		chainId,
		collectionAddress,
		includeMetadata: false
	});
	const mintedTokenIds = new Set(tokenSupplies?.tokenIDs?.filter((token) => BigInt(token.supply) > 0n).map((token) => token.tokenID) ?? []);
	...
	const result = useInfiniteQuery({ ...queryOptions$1 });
	...
	const shouldFetchNextPage = result.hasNextPage && (filteredTokenMetadata?.length ?? 0) < (mintedTokenIds?.size ?? 0);
	return {
		...result,
...
function useList721ShopCardData({ tokenIds, chainId, contractAddress, salesContractAddress, enabled = true }) {
	...
	const { data: mintedTokensMetadata, isLoading: mintedTokensMetadataLoading } = useSearchMintedTokenMetadata({
		chainId,
		collectionAddress: contractAddress,
	...
	if (mintedTokensMetadata?.tokenMetadata) for (const metadata of mintedTokensMetadata.tokenMetadata) mintedTokensMetadataMap.set(metadata.tokenId, metadata);
...
//#endregion
export { AlertMessage, DatabeatAnalytics, MODAL_CONTENT_PROPS, MODAL_OVERLAY_PROPS, MarketplaceProvider, MarketplaceQueryClientProvider, MarketplaceSdkContext, collectionDetailsPollingOptions, dateToUnixTime, generateCancelTransaction, generateListingTransaction, generateOfferTransaction, generateSellTransaction, listOffersForCollectibleOptions, sellModal$, switchChainModal_default, useAnalytics, useAutoSelectFeeOption, useBalanceOfCollectible, useCancelOrder, useCancelTransactionSteps, useCheckoutOptions, useCheckoutOptionsSalesContract, useCollectible, useCollection, useCollectionBalanceDetails, useCollectionDetails, useCollectionDetailsPolling, useComparePrices, useConfig, useConnectorMetadata, useConvertPriceToUSD, useCountListingsForCollectible, useCountOfCollectables, useCountOfPrimarySaleItems, useCountOffersForCollectible, useCurrency, useERC721SaleMintedTokens, useFilterState, useFilters, useFiltersProgressive, useFloorOrder, useGenerateCancelTransaction, useGenerateListingTransaction, useGenerateOfferTransaction, useGenerateSellTransaction, useGetCountOfPrimarySaleItems, useGetReceiptFromHash, useGetTokenRanges, useHighestOffer, useInventory, useList1155SaleSupplies, useList1155ShopCardData, useList721ShopCardData, useListBalances, useListCollectibleActivities, useListCollectibles, useListCollectiblesPaginated, useListCollectionActivities, useListCollections, useListListingsForCollectible, useListMarketCardData, useListOffersForCollectible, useListPrimarySaleItems, useListShopCardData, useListTokenMetadata, useLowestListing, useMarketCurrencies, useMarketplaceConfig, useOpenConnectModal$1 as useOpenConnectModal, useOrderSteps, useRoyalty, useSearchMintedTokenMetadata, useSearchTokenMetadata, useSellModal, useShopCollectibleSaleData, useSwitchChainModal, useSwitchChainWithModal, useTokenSaleDetailsBatch, useTokenSupplies, useTransferTokens, useWallet };
//# sourceMappingURL=hooks-oBp09Mpg.js.map