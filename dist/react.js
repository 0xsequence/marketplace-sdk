...
import { c as getSequenceNodeGatewayUrl, g as getQueryClient, l as getTrailsApiUrl, n as getIndexerClient, o as getSequenceApiUrl, r as getMarketplaceClient, s as getSequenceIndexerUrl, u as indexerURL } from "./api.js";
...
function useBuyTransaction(options) {
	...
	const marketQuery = useMarketTransactionSteps({
		...
		enabled: transactionType === TransactionType$2.MARKET_BUY
	});
	const primaryQuery = usePrimarySaleTransactionSteps({
		...
		contractType,
		enabled: transactionType === TransactionType$2.PRIMARY_SALE
	});
...
const useBuyModalData = () => {
	...
	const { data: primarySaleItemData, isLoading: primarySaleItemLoading, isError: primarySaleItemError } = usePrimarySaleItem({
		chainId,
	...
	return {
		...
		isError: collectableError || ordersError || primarySaleItemError || currencyError || collectionError
	};
};
...
function useBuyModalContext() {
	...
	const { collectible, collection, currency, marketOrder, collectionAddress, primarySaleItem, isLoading: isBuyModalDataLoading, isMarket, isShop } = useBuyModalData();
...
function useCryptoPaymentModalContext({ chainId, steps, onSuccess }) {
	...
	return {
		...
		flags: { isMarket },
		permissions: {
			canApprove: hasSufficientBalance && !isLoadingBalance && !isLoadingBuyModalData && !isAnyTransactionPending,
			canBuy: hasSufficientBalance && !isLoadingBalance && !isLoadingBuyModalData && (isSequenceConnector ? true : !approvalStep) && !isAnyTransactionPending
		},
		price: {
			formattedPrice,
			renderCurrencyPrice,
...
const BuyModalContent = () => {
	const { config, modalProps, close, steps, primarySaleItem, marketOrder, collectible, buyStep, isLoading, collection, checkoutMode, formattedAmount, isShop, handleTrailsSuccess, handleTransactionSuccess } = useBuyModalContext();
	...
	return /* @__PURE__ */ jsx(Modal, {
		...
		children: /* @__PURE__ */ jsxs("div", {
			className: "relative flex grow flex-col items-center p-6",
			children: [
				/* @__PURE__ */ jsx(Text, {
					className: "w-full text-center font-body font-bold text-large text-text-100",
