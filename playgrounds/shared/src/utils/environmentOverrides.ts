...
export function getMarketplaceApiUrl(
	...
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return (
		config._internal?.overrides?.api?.marketplace?.url || urls.marketplaceApi
	);
}
...
export function getSequenceApiUrl(
	...
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return config._internal?.overrides?.api?.sequenceApi?.url || urls.api;
}

export function getSequenceWalletUrl(
	config: SdkConfig,
	defaultEnv: 'production' | 'development',
): string {
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return config._internal?.overrides?.api?.sequenceWallet?.url || urls.api;
}

export function getNodeGatewayUrl(
	config: SdkConfig,
	defaultEnv: 'production' | 'development',
): string {
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return config._internal?.overrides?.api?.nodeGateway?.url || urls.api;
}
...
export function getMetadataUrl(
	...
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return config._internal?.overrides?.api?.metadata?.url || urls.metadata;
}
...
export function getIndexerUrl(
	...
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return config._internal?.overrides?.api?.indexer?.url || urls.indexer;
}
...
export function getBuilderUrl(
	...
	const urls = ENVIRONMENT_URLS[defaultEnv];
	return config._internal?.overrides?.api?.builder?.url || urls.builder;
}
