...
export function PrivyOverridesSettings() {
	...
	function toggleConnect() {
		if (authenticated && address) {
			// User is connected, disconnect them
			disconnect();
			logout();
		} else {
			// User is not connected, open Privy login modal
			login();
		}
	}
				...
				<div className="flex flex-col gap-3">
					...
					<TextInput
						...
						controls={
							<div>
								<Button
									label={authenticated && address ? 'Disconnect' : 'Connect'}
									size="xs"
									shape="square"
