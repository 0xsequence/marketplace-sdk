...
import { StepType } from '../src/react/_internal';
...
const marketplaceHandlers = {
	success: [
		...
		http.post(
			/.*marketplace-api\.sequence\.app\/rpc\/Marketplace\/GenerateSellTransaction/,
			({ request }) => {
				console.log(
					'MSW: GenerateSellTransaction request intercepted',
					request.headers.get('x-access-key'),
				);
				return HttpResponse.json({
					steps: [
