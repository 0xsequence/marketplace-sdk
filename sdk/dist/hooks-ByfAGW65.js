...
import {
	...
	UserRejectedRequestError as UserRejectedRequestError$1,
...
function MarketplaceProvider({ config, children, openConnectModal }) {
	...
	if (openConnectModal) {
		...
		return /* @__PURE__ */ jsx(MarketplaceQueryClientProvider, {
			children: /* @__PURE__ */ jsx(MarketplaceSdkContext.Provider, {
				value: context,
...
var AutoSelectFeeOptionError = /* @__PURE__ */ ((
	AutoSelectFeeOptionError$1,
) => {
	...
	AutoSelectFeeOptionError$1['FailedToCheckBalances'] =
		'Failed to check balances';
	AutoSelectFeeOptionError$1['InsufficientBalanceForAnyFeeOption'] =
...
function useBalanceOfCollectible(args) {
	...
	const collection = marketplaceConfig?.market.collections.find(
		(collection$1) => collection$1.itemsAddress === args.collectionAddress,
	);
			...
			try {
				...
				switch (error.name) {
					case 'SwitchChainNotSupportedError':
						throw new ChainSwitchError(await wallet$1.getChainId(), chainId);
		...
		handleSignMessageStep: async (stepItem) => {
			try {
				...
				if (error.cause instanceof BaseError$1) {
					...
					if (viemError instanceof UserRejectedRequestError$1)
						throw new UserRejectedRequestError();
				}
...
const useCancelTransactionSteps = ({
	collectionAddress,
	chainId,
	callbacks,
	setSteps,
	...
	const checkAndSwitchChain = async () => {
		...
		return new Promise((resolve, reject) => {
			if (chainIdMismatch)
				if (isWaaS) switchChain().then(resolve).catch(reject);
				else
	...
	const executeTransaction = async ({ transactionStep }) => {
		const hash = await wallet$1?.handleSendTransactionStep(
			Number(chainId),
			transactionStep,
		);
		return hash;
...
function useCheckoutOptionsSalesContract(params) {
	...
	const defaultConfig = useConfig();
	const queryOptions$1 = checkoutOptionsSalesContractQueryOptions(
		params === skipToken
			? {
...
async function fetchConvertPriceToUSD(params) {
	...
	const currencyDetails = currencies.find(
		(c) => c.contractAddress.toLowerCase() === currencyAddress.toLowerCase(),
	);
...
function useERC721SaleMintedTokens({
	chainId,
	contractAddress,
		...
		() => ({
			...
			toggleStringFilterValue: (name, value) => {
				...
				const existingValues =
					filter?.type === PropertyType.STRING ? (filter.values ?? []) : [];
...
function useList721ShopCardData({
	tokenIds,
	chainId,
	contractAddress,
	salesContractAddress,
	...
	const collectibleCards = tokenIds.map((tokenId) => {
		...
		const quantityInitial =
			...
			(saleDetails?.supplyCap ? saleDetails.supplyCap.toString() : void 0);
...
function useList1155ShopCardData({
	tokenIds,
	chainId,
	contractAddress,
	salesContractAddress,
...
function useListMarketCardData({
	collectionAddress,
	chainId,
	orderbookKind,
	collectionType,
	...
	const collectibleCards = useMemo(() => {
		return allCollectibles.map((collectible) => {
			const balance = collectionBalance?.balances.find(
				(balance$1) => balance$1.tokenID === collectible.metadata.tokenId,
			)?.balance;
			const cardProps = {
...
function useShopCollectibleSaleData({
	chainId,
	salesContractAddress,
...
const prepareTransferConfig = (params, accountAddress) => {
	if (params.contractType === 'ERC721')
		return {
			...
			args: [accountAddress, params.receiverAddress, BigInt(params.tokenId)],
		};
	return {
		abi: ERC1155_ABI,
		address: params.collectionAddress,
...
export {
	...
	useOpenConnectModal$1 as useOpenConnectModal,
	useRoyalty,
	useSellModal,
...
//# sourceMappingURL=hooks-ByfAGW65.js.map
